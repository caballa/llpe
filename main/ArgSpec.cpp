...
static Constant* getStringPtrArray(std::string& bytes, std::vector<size_t>& lineStarts, Module& M) {
  ...
  for(unsigned i = 0; i < lineStarts.size(); ++i) {
	...
	ConstantExpr::getGetElementPtr(
		 cast<PointerType>(EnvInitG->getType())->getElementType(),
		 EnvInitG, gepArgs, 2));
  ...
  Constant* EnvPtrsPtr = ConstantExpr::getGetElementPtr(
			 cast<PointerType>(EnvPtrsG->getType())->getElementType(),
			 EnvPtrsG, gepArgs, 2);
  ...
  for(unsigned i = 0; i < argc; ++i) {
    ...
    if(lineStarts[i] != -1) {
      ...
      Constant* stringPtr = ConstantExpr::getGetElementPtr(
			      cast<PointerType>(ArgvConsts->getType())->getElementType(),
      ...
      Instruction* argvPtr =
	GetElementPtrInst::Create(cast<PointerType>(Arg->getType())->getElementType(),
