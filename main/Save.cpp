...
bool IntegrationAttempt::emitVFSCall(ShadowBB* BB, ShadowInstruction* I, SmallVector<CommittedBlock, 1>::iterator& emitBBIter) {
...

  {
	...
	if(it->second.isFifo) {
	  ...
	  Type* Int32Ty = IntegerType::get(Context, 32);
	  Constant* CheckFn = F.getParent()->getOrInsertFunction("lliowd_ok",
	...
	Constant* CopySource =
	  ConstantExpr::getGetElementPtr(
			    cast<PointerType>(ArrayGlobal->getType())->getElementType(),
			    ArrayGlobal, Idxs, 2);
...
Constant* llvm::getGVOffset(Constant* GV, int64_t Offset, Type* targetType) {
  ...
  if(Offset == 0)
    ...
    OffsetGV = ConstantExpr::getGetElementPtr(
			      cast<PointerType>(CastGV->getType())->getElementType(),
			      CastGV, OffC);
  }
...
bool IntegrationAttempt::synthCommittedPointer(ShadowValue* I, Type* targetType, ImprovedVal IV, BasicBlock* emitBB, Value*& Result) {
  ...
  if(Base.isGV()) {
    ...
    Value* OffsetI = GetElementPtrInst::Create(
				cast<PointerType>(CastI->getType())->getElementType(),
...
void InlineAttempt::commitArgsAndInstructions() {
  ...
  if(isCommitted()) {
	...
	targetCallInfo && targetCallInfo->targetStackDepth == (uint32_t)pass->llioPreludeStackIdx) ||
      ...
      Constant* WDInit = getGlobalModule()->getOrInsertFunction("lliowd_init",
